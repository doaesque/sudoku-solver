...
export const useSudokuGame = () => {
  ...
  // FIX ERROR 2: Update ref di dalam useEffect, bukan saat render
  useEffect(() => {
  ...
  const handleSolve = async () => {
    ...
    // FIX ERROR 1: Gunakan tempBoard (copy) untuk validasi, jangan ubah state 'board' langsung
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        if (board[r][c] !== 0 && cellStatus[r][c] !== 'fixed') {
          const val = board[r][c];

          // Buat salinan board untuk pengecekan
          const tempBoard = board.map(row => [...row]);
          tempBoard[r][c] = 0; // Kosongkan di salinan

          if (!isValid(tempBoard, r, c, val)) {
            setPanelMsg(`⚠️ Input salah di baris ${r+1} kolom ${c+1}! Perbaiki dulu.`);
            const ns = [...cellStatus]; ns[r][c] = 'error'; setCellStatus(ns);
            return;
          }
        }
      }
    ...
    // Kirim salinan board ke solver
    const boardToSolve = board.map(r => [...r]);

    // Fungsi update visual wrapper
  ...
  const handleSolveCell = () => {
    if (isSolving) return;
    if (!solutionKey) { setPanelMsg("⚠️ Pilih level dulu!"); return; }
    if (!activeCell) { setPanelMsg("⚠️ Klik kotak yang mau diisi!"); return; }

    const { r, c } = activeCell;
    if (cellStatus[r][c] === 'fixed') { setPanelMsg("⛔ Ini angka soal!"); return; }
  ...
  const handleHint = () => {
    if (isSolving) return;
    if (!initialBoard) { setPanelMsg("⚠️ Pilih level dulu!"); return; }
      ...
      for (let c = 0; c < 9; c++) {
        if (board[r][c] !== 0 && cellStatus[r][c] !== 'fixed') {
          ...
          if (solutionKey) {
             ...
             // Fallback jika belum ada key (misal input manual), pakai validasi logika
             const val = board[r][c];
             const tempBoard = board.map(row => [...row]);
             tempBoard[r][c] = 0;
  ...
  const handleInputChange = (e, r, c) => {
    ...
    if (cellStatus[r][c] === 'error' || cellStatus[r][c] === 'trial' || cellStatus[r][c] === '') {
       const ns = [...cellStatus]; ns[r][c] = 'user-filled'; setCellStatus(ns);
    }
